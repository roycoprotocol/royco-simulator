...
7. Day keeps Dawn accounting and chart behavior but exposes a deliberately compact public surface. Display only Base strategy APY, minimum coverage, minimum liquidity, Junior yield share, LP yield share, observation-period duration, and the Conservative/Balanced/Aggressive ladder. Protected exit, Junior sizing/replenishment, curve endpoints, notional sizing, liquidation bonus, fees, and pool mechanics are backend market configuration. Public outputs are the full-detail chart, the three tranche yields, and base-strategy return.
