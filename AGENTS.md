...
4. Every public Day route must use `StrictDaySimulatorPageShell`. It may not pass variants, classes, styles, copy, or custom children. This governs *market* routes — a page presenting a specific Day market. `/create` is exempt: it is a configuration wizard, not a market, has no `market.json`, and is invisible to `scripts/day-simulator/verify.mjs`. Its code lives entirely in `app/create/`, `lib/pool-creator/` and `components/pool-creator/`, and it may not modify anything under the template lock. See `docs/POOL_CREATOR.md`.
