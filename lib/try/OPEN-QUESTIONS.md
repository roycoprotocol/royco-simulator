...
## Modeling decisions (confirmed by product owner)

**Junior replenishment / maintained coverage (2026-07-06).** The accountant with FIXED Junior capital leaves Senior nakedly exposed once a deep drawdown exhausts Junior (Junior stays wiped, Senior tracks the strategy down — e.g. the 2021 path gave Senior −18%). The intended product does NOT work that way: **Senior can take a step-down if a drawdown drains Junior within an observation period (that becomes Senior's new mark), but once the term ends and the market reopens (PERPETUAL), fresh Junior capital is attracted to restore the ≥30% buffer, re-protecting Senior from the new level.** `backtest.ts` implements this as the default (`maintainJuniorCoverage: true`): after each PERPETUAL sync where Junior is below the target buffer, it deposits fresh Junior to restore coverage to the YDM target, minting Junior shares at the current share price (so per-unit Junior index is undistorted). This reproduces the brief's verified 2021 Senior result (~$145 / +7%/yr vs the Excel's $149 / +7.4%/yr; the ~3% gap is the accountant's exact premium math vs the Excel ratchet). Honest boundary retained: a crash exceeding the full buffer *within a single observation window* still marks Senior down. Knob: replenish-to-target (33%) vs replenish-to-minimum (30%) — currently target; confirm if 30% preferred.
