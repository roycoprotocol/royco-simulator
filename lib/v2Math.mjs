...
export const rawNavFromState = (state, assetPrice, quotePrice) => {
  ...
  const perShareRaw = stShares > 0
    ? (assetPrice * stAssets) / stShares
    : 0;

  const ST_RAW_NAV = stShares > 0
    ? assetPrice * stAssets * externalShares / stShares
    : 0;
  const shareNavInPool = stShares > 0
    ? assetPrice * stAssets * internalShares / stShares
    : 0;
  const quoteNavInPool = quotePrice * state.quoteReserves;
  const JT_RAW_NAV = shareNavInPool + quoteNavInPool;
  const totalNav = assetPrice * stAssets + quoteNavInPool;
...
// Coverage / utilization basis valued at the pool's balance point (oracle par).
// Each swap pushes cash in/out of the pool, so measuring requiredCoverage on the
// instantaneous reserves makes utilization slide as the price swings off balance.
// Instead we recover the E-CLP invariant from the current reserves and value the
// reserves that same invariant implies at `balancePoolPrice` — the basis then
// ratchets up with real σ/fee gains (invariant grows) but does not oscillate when
// you swap back and forth. Returns totalNav at balance = (all ST shares at oracle)
// + balanced pool quote.
export const coverageNavAtBalance = (state, assetPrice, quotePrice, eclpConfig, balancePoolPrice = 1) => {
  const raw = rawNavFromState(state, assetPrice, quotePrice);
  const cfg = eclpConfig?.float;
  if (!cfg || !(raw.shareNavInPool > 0) || !(raw.quoteNavInPool > 0)) {
    return raw.totalNav; // no E-CLP curve to rebalance along → fall back to instantaneous
  }
  const currentPrice = computeEclpSpotPrice(
    raw.shareNavInPool, raw.quoteNavInPool, cfg.alpha, cfg.beta, cfg.lambda, cfg.phi,
  );
  ...
  if (!unitNow || !unitBal || !(unitNow.shareNav > 0)) return raw.totalNav;
  const invariantScale = raw.shareNavInPool / unitNow.shareNav; // E-CLP invariant (L)
  const balQuoteNav = invariantScale * unitBal.quoteNav;
  const stSharesNav = raw.totalNav - raw.quoteNavInPool; // assetPrice * stShares (composition-independent)
  return stSharesNav + balQuoteNav;
};
...
export const syncAccountingOnBefore = (state, assetPrice, quotePrice, ydmShare) => {
  const currentRaw = rawNavFromState(state, assetPrice, quotePrice);
...
export const syncAccountingOnAfter = (before, afterMutation, assetPrice, quotePrice, ydmShare) => {
  const newRaw = rawNavFromState(afterMutation, assetPrice, quotePrice);

  const oldExternal = Math.max(0, before.stShares - before.internalShares);
  const newExternal = Math.max(0, before.stShares - afterMutation.internalShares);

  let stRawCheckpoint = 0;
  let stEffectiveNav = 0;
  let stIL = 0;
  const jtRawCheckpoint = Math.max(0, before.jtRawCheckpoint);
  let jtEffectiveNav = 0;
  let jtIL = 0;

  if (oldExternal > NAV_EPS) {
    const scalingFactor = newExternal / oldExternal;
    stRawCheckpoint = Math.max(0, before.stRawCheckpoint * scalingFactor);
    stEffectiveNav = Math.max(0, before.stEffectiveNav * scalingFactor);
    stIL = Math.max(0, before.stIL * scalingFactor);
    jtIL = Math.max(0, before.jtIL * scalingFactor);
  } else {
    stRawCheckpoint = Math.max(0, newRaw.ST_RAW_NAV);
    stEffectiveNav = stRawCheckpoint;
    stIL = 0;
    jtIL = 0;
  }

  jtEffectiveNav = Math.max(0, stRawCheckpoint + jtRawCheckpoint - stEffectiveNav);

  const recomposedCheckpoint = {
    stRawCheckpoint,
    jtRawCheckpoint,
    stEffectiveNav,
    jtEffectiveNav,
  ...
  const waterfall = applyDuskWaterfall(recomposedCheckpoint, newRaw, ydmShare);
