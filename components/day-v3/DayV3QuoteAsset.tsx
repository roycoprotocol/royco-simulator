  ...
  return (
    <div className="flex min-w-0 flex-col gap-3 rounded-xl border border-[var(--border-subtle)] bg-[var(--card)] px-4 py-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[10.5px] leading-relaxed text-[var(--tertiary)]">
            The pool&apos;s quote asset. It is the other side of every immediate
            exit, it is most of what the SLP holds at rest, and it appears as the
            exit asset in the position comparison.
          </p>
        </div>
      ...
      <label className="flex min-w-0 flex-col gap-1.5">
        ...
        <input
          className="min-w-0 rounded-lg border border-[var(--border-subtle)] bg-[var(--foundation)] px-3 py-2 font-mono text-[15px] font-semibold leading-none outline-none placeholder:font-sans placeholder:text-[12px] placeholder:font-normal placeholder:text-[var(--tertiary)] focus:border-[var(--foreground)]"
          maxLength={24}
          onBlur={() => {
      ...
      <DayV3NumberField
        ...
        max={30}
        min={0}
        note="Enter 0 for a quote asset that does not accrue. Anything above 0 is paid to the SLP, which holds the quote side, and is shown as exit-asset carry in the position comparison. Exact pool sizing for a yield-bearing quote asset is finalized in Royco Deploy."
        onChange={onYieldPct}
        origin={yieldOrigin}
