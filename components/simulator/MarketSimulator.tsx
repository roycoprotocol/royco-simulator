...
// ---------------------------------------------------------------------------
// Shared simulator UI. Market-specific data and copy enter only through the
// SimulatorMarket definition. Every accounting number comes from runBacktest()
// and the verified shared Royco Day engine; this component contains no forked
// waterfall, coverage, NAV, or YDM math.
// ---------------------------------------------------------------------------
...
function safeBacktest(
  run: () => BacktestResult,
  emptyResult: BacktestResult,
): { result: BacktestResult; error: string | null } {
  try {
    return { result: run(), error: null };
  } catch (e) {
    return { result: emptyResult, error: e instanceof Error ? e.message : String(e) };
  }
}
...
export default function MarketSimulator({
  ...
  const codec = useMemo(() => createPermalinkCodec(market), [market]);
  ...
  const initial = useMemo(
    () => codec.stateFromQuery(queryFromRecord(initialQuery)),
    [codec, initialQuery],
  );

  const [params, setParams] = useState<SimulatorParams>(initial.params);
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [maintainCoverage, setMaintainCoverage] = useState(initial.maintain);
  const [showHistory, setShowHistory] = useState(true);

  const config = useMemo(() => buildSimulatorConfig(params), [params]);
  ...
  const maxIndex = market.series.length - 1;
  ...
  const windowSeries = useMemo(
    () => market.series.slice(view.a, view.b + 1),
    [market.series, view.a, view.b],
  );
  ...
  const run = useMemo(
    ...
    [config, emptyResult, params.depositST, params.depositJT, maintainCoverage, windowSeries],
  );
  ...
  const exposedResult = useMemo(
    ...
    [config, emptyResult, params.depositST, params.depositJT, windowSeries],
  );
  ...
  const activePreset = useMemo(() => findMarketPreset(market, params), [market, params]);
  ...
  // Every preset runs through the real engine. The results drive the ladder prose rather
  // than a persistent wall of status badges. Screened over the ACTIVE WINDOW, so the prose
  // describes the market currently on screen rather than history the user windowed away.
  const presetScreen = useMemo(
    () => screenMarketPresets(market, windowSeries),
    [market, windowSeries],
  );
  ...
  // params, and always shows the market's complete input series.
  const navResult = useMemo(
    ...
    [config, emptyResult, market.series, params.depositST, params.depositJT, maintainCoverage],
  );
  ...
  const allDates = useMemo(() => market.series.map((point) => point.date), [market.series]);
  ...
  const updateParam = (patch: Partial<SimulatorParams>) =>
    setParams((p) => {
      const next = { ...p, ...patch };
      const relinked =
  ...
  const permalink = (): string => {
    ...
    return `${window.location.origin}${window.location.pathname}?${codec.queryFromState(params, maintainCoverage, view)}`;
  };
  ...
  const deployText = useMemo(() => {
    ...
    return [
      ...
      `Generated by ${market.copy.title}`,
      `Loaded market: ${market.copy.loadedMarket}`,
      ...
      '',
      'Suggested labels',
      `seniorTrancheName: ${market.copy.seniorTrancheName}`,
      `seniorTrancheSymbol: ${market.copy.seniorTrancheSymbol}`,
      `juniorTrancheName: ${market.copy.juniorTrancheName}`,
      `juniorTrancheSymbol: ${market.copy.juniorTrancheSymbol}`,
      '',
      ...
      `backtestWindowPoints: ${result.steps.length}${viewIsFull ? '   // full available history' : `   // windowed; full history is ${allDates[0]} to ${allDates[allDates.length - 1]}`}`,
    ...
    <div className="flex flex-col" style={{ gap: 10 }}>
      ...
      >
        ...
        {!seniorProtected && (
          ...
          >
            ...
            {result.seniorLossEvents.length} Senior loss event
            {result.seniorLossEvents.length === 1 ? '' : 's'} across{' '}
            {viewIsFull ? 'full history' : 'the selected window'} ({fmtSignedPct(-result.seniorMaxDrawdown, 1)} worst
            Senior drawdown). Senior is the protected tranche, so these terms do not deliver
            what Senior is being sold.
          </div>
        )}
          ...
          <div className="mt-4 flex flex-col gap-4">
            ...
            <div>
              ...
              <p className="mt-1.5" style={{ color: C.kpiLabel, fontSize: 11, lineHeight: 1.4 }}>
                ...
                Senior yield to Junior. Each rung is recomputed through the accountant; the compact
                warning above appears only when the selected terms produce a Senior loss.
              </p>
            </div>
        ...
        {showHistory && (
          <div className="mt-4">
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 mb-3" style={{ fontSize: 11.5, color: C.muted }}>
              ...
              <LegendSwatch color={C.strategyLine}>{market.copy.strategyLegend}</LegendSwatch>
            ...
            <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div style={{ border: `1px solid ${C.border}`, padding: '12px 14px' }}>
                ...
                {/* Prose is derived from the same runs as the preset screen (presetProse), so a
      ...
      >
        ...
        <p className="mt-3" style={{ color: C.kpiLabel, fontSize: 11, lineHeight: 1.45 }}>
          {market.certification.detail} Parameters are illustrative and pending accountant
          sign-off. Projections, not promises. This is not an offer or investment advice.
        </p>
      </section>
      ...
      >
        <p style={{ borderColor: C.border }}>
          ...
          {market.copy.footerParagraphs[0]}
        </p>
        <p className="mt-1">
          {market.copy.footerParagraphs[1]}
        </p>
        <p className="mt-1">
          {market.copy.footerParagraphs[2]}
        </p>
      </footer>
    </div>
  );
