...
function Badge({ children, tone = "input" }: { children: React.ReactNode; tone?: "input" | "output" | "context" }) {
  ...
  return (
    <span style={{ ...colors, borderRadius: 999, display: "inline-flex", fontFamily: MONO, fontSize: 9.5, fontWeight: 800, letterSpacing: "0.08em", padding: "5px 8px", textTransform: "uppercase" }}>
      {children}
    </span>
  );
}
...
function RangeControl({
  ...
  return (
    <label style={{ display: "block" }}>
      ...
      <span className="mt-1.5 block" style={{ color: C.muted, fontSize: 12, lineHeight: 1.45 }}>{description}</span>
    </label>
  );
}
    ...
    <div className="flex flex-col" style={{ gap: 14, fontFamily: SANS }}>
      <section style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, overflow: "hidden" }}>
        <div className="flex flex-col gap-3 border-b px-4 py-4 sm:px-6 lg:flex-row lg:items-end lg:justify-between" style={{ borderColor: C.border }}>
          ...
          <div className="flex items-center gap-3">
            <p style={{ color: C.muted, fontSize: 12 }}><strong style={{ color: C.text }}>SLP (Senior Liquidity Provider)</strong> = the LP capital that buys ST from sellers.</p>
            <button disabled={!changed} onClick={reset} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, color: C.rust, fontSize: 12, fontWeight: 650, minHeight: 36, opacity: changed ? 1 : 0.45, padding: "7px 11px", whiteSpace: "nowrap" }} type="button">Reset inputs</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,.92fr)_minmax(390px,1.08fr)]">
          <div className="flex flex-col gap-5 p-4 sm:p-6 lg:border-r" style={{ borderColor: C.border }}>
            <div>
              ...
              <div className="mt-3">
                <RangeControl description="Market-wide pool setting—not a minimum wallet size." display={`${minLiquidityPct.toFixed(0)} for every 100 ST`} label="Pool capital" max={50} min={1} onChange={onMinLiquidityChange} step={1} tone={C.green} value={minLiquidityPct} />
              </div>
              <div aria-label={`For every 100 units of ST, the market requires ${minLiquidityPct.toFixed(0)} units of SLP capital`} className="mt-3" role="img">
                <div className="flex h-10 overflow-hidden rounded-lg" style={{ border: `1px solid ${C.border}` }}>
                  <div className="flex items-center px-3" style={{ background: C.rustSoft, color: C.rust, width: `${stVisualShare}%` }}><span style={{ fontFamily: MONO, fontSize: 11, fontWeight: 800, whiteSpace: "nowrap" }}>100 ST</span></div>
                  <div className="flex items-center justify-end px-2" style={{ background: C.green, color: C.card, minWidth: 52, width: `${slpVisualShare}%` }}><span style={{ fontFamily: MONO, fontSize: 11, fontWeight: 800, whiteSpace: "nowrap" }}>{minLiquidityPct.toFixed(0)} SLP</span></div>
                </div>
              </div>
            </div>
            ...
            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 18 }}>
              ...
              <div className="mt-3 grid grid-cols-4 gap-1.5" role="group" aria-label="Price range presets">
                {[0.5, 1, 3, 10].map((band) => {
                  ...
                  return <button aria-pressed={active} key={band} onClick={() => onEclpBandWidthChange(band)} style={{ background: active ? C.green : C.page, border: `1px solid ${active ? C.green : C.border}`, borderRadius: 8, color: active ? C.card : C.text, fontFamily: MONO, fontSize: 11, fontWeight: 750, minHeight: 38 }} type="button">{formatBand(band)}%</button>;
                })}
              </div>
              <div aria-label={`The configured price range runs from 100 percent marked value to ${lowerBandPct.toFixed(1)} percent`} className="mt-3 grid grid-cols-[auto_1fr_auto] items-center gap-3" role="img">
                <span style={{ color: C.text, fontFamily: MONO, fontSize: 11, fontWeight: 750 }}>100%</span>
                ...
                <span style={{ color: C.green, fontFamily: MONO, fontSize: 11, fontWeight: 750 }}>{lowerBandPct.toFixed(1)}%</span>
              </div>
            ...
            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 18 }}>
              ...
              <div className="mt-3">
                <RangeControl description="Your share of ST yield is one input to SLP return—not the return itself." display={`${liqSharePct.toFixed(0)}% of ST yield`} label="Your share of ST yield" max={80} min={0} onChange={onLiquidityShareChange} step={1} tone={C.green} value={liqSharePct} />
              </div>
              <div aria-label={`ST retains ${retainedShare.toFixed(0)} percent of source yield, JT receives ${riskSharePct.toFixed(0)} percent, and SLP receives ${liqSharePct.toFixed(0)} percent`} className="mt-3" role="img">
                ...
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1" style={{ color: C.muted, fontSize: 10.5 }}>
                  <span><i aria-hidden style={{ background: C.tan, borderRadius: 2, display: "inline-block", height: 7, marginRight: 5, width: 7 }} />ST keeps {retainedShare.toFixed(0)}%</span>
                  <span><i aria-hidden style={{ background: C.rust, borderRadius: 2, display: "inline-block", height: 7, marginRight: 5, width: 7 }} />JT gets {riskSharePct.toFixed(0)}%</span>
            ...
            <details style={{ background: C.page, border: `1px solid ${C.border}`, borderRadius: 11, padding: "11px 13px" }}>
              <summary style={{ color: C.text, cursor: "pointer", fontSize: 12.5, fontWeight: 650 }}>Advanced market inputs</summary>
              <div className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2">
                <RangeControl description="First-loss capital protecting ST; not a guarantee for SLP." display={`${coveragePct.toFixed(0)}%`} label="JT first-loss capital" max={65} min={3} onChange={onCoverageChange} step={1} value={coveragePct} />
                <RangeControl description="Separate yield share paid to JT for first-loss risk." display={`${riskSharePct.toFixed(0)}%`} label="JT share of ST yield" max={80} min={0} onChange={onRiskShareChange} step={1} tone={C.ink} value={riskSharePct} />
                ...
                <RangeControl description="Time allowed for source recovery before a covered JT loss is finalized." display={`${observationDays} days`} label="Recovery window" max={194} min={7} onChange={onObservationDaysChange} step={1} value={observationDays} />
                <label className="flex items-start gap-3 sm:col-span-2" style={{ color: C.muted, fontSize: 12, lineHeight: 1.45 }}>
                  ...
                  <span><strong style={{ color: C.text, display: "block", fontWeight: 650 }}>Restore JT after finalized losses</strong>Fresh JT capital rebuilds the configured first-loss buffer.</span>
                </label>
              </div>
            </details>
          </div>
          ...
          <div className="p-4 sm:p-6">
            <div className="lg:sticky lg:top-4">
              <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2"><Badge tone="output">Output</Badge><Eyebrow>Your SLP result</Eyebrow></div>{changed && <span style={{ color: C.green, fontFamily: MONO, fontSize: 10.5, fontWeight: 750 }}>Updated live</span>}</div>
              <div className="mt-3 rounded-xl" style={{ background: C.ink, color: C.card, padding: 18 }}>
                <p style={{ color: "#B8B2A9", fontFamily: MONO, fontSize: 9.5, letterSpacing: "0.09em", textTransform: "uppercase" }}>What 100 SLP did in this test</p>
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-2"><p style={{ color: "#B8B2A9", fontSize: 11 }}>Modeled outcome—not a promised or fixed return.</p><p style={{ color: "#D6D2CA", fontSize: 10.5 }}>{changed ? `${deltaLabel(((slp?.apy ?? 0) * 100) - baseline.slpApyPct)} annualized from start` : "Starting setup"}</p></div>
              </div>

              <div className="mt-5">
                <div className="flex items-center justify-between gap-3"><Eyebrow>One ST sale through your pool</Eyebrow><Badge tone="context">Focused view</Badge></div>
                <div aria-label={`One trade can sell ${nearParPct.toFixed(1)} percent of ST near marked value before continuing to the ${atomicPct.toFixed(1)} percent pool limit`} className="mt-4" role="img">
                  <div className="mb-2 flex items-center justify-between gap-3" style={{ color: C.muted, fontSize: 10.5 }}><span>ST enters the pool</span><span>Stable value leaves → seller</span></div>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-4">
                    <div><p style={{ color: C.green, fontFamily: MONO, fontSize: 15, fontWeight: 800 }}>{nearParPct.toFixed(1)}% of ST</p><p style={{ color: C.muted, fontSize: 10.5 }}>near marked value · {(liquidity.referenceQuote.slippage * 100).toFixed(1)}% avg slippage</p></div>
                    <div style={{ textAlign: "right" }}><p style={{ color: C.rust, fontFamily: MONO, fontSize: 15, fontWeight: 800 }}>{atomicPct.toFixed(1)}% of ST</p><p style={{ color: C.muted, fontSize: 10.5 }}>one-trade pool limit · {(liquidity.boundaryQuote.slippage * 100).toFixed(1)}% avg slippage</p></div>
                  </div>
                </div>
      ...
      <section style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "20px clamp(16px,2.5vw,28px)" }}>
        ...
        <div aria-label={`The modeled SLP return combines a ${liqSharePct.toFixed(0)} percent share of ST yield, ${modelAssumptions.stableYieldPct.toFixed(1)} percent stable-side yield, ${modelAssumptions.swapFeeBps.toFixed(0)} basis point swap fees at ${modelAssumptions.poolTurnoverPerYear.toFixed(1)} times annual turnover, and pool trading outcomes`} className="mt-5 grid grid-cols-1 gap-3 lg:grid-cols-[1.2fr_auto_.7fr_auto_.85fr] lg:items-center" role="img">
          <div className="grid grid-cols-2 gap-2">
            ...
            ].map(([label, value]) => <div key={label} style={{ background: C.page, border: `1px solid ${C.border}`, borderRadius: 10, padding: 12 }}><p style={{ color: C.faint, fontSize: 10.5 }}>{label}</p><p className="mt-1" style={{ color: C.text, fontFamily: MONO, fontSize: 12.5, fontWeight: 750 }}>{value}</p></div>)}
          </div>
          <div aria-hidden className="hidden text-2xl lg:block" style={{ color: C.faint }}>→</div>
          <div style={{ background: C.greenSoft, border: "1px solid #C6DCCB", borderRadius: 999, padding: "22px 16px", textAlign: "center" }}><p style={{ color: C.green, fontFamily: MONO, fontSize: 12, fontWeight: 800 }}>SLP POOL</p><p className="mt-1" style={{ color: C.muted, fontSize: 10.5 }}>{modelAssumptions.reinvestLiquidityPremium ? "Premium reinvested" : "Premium paid out"}</p></div>
          <div aria-hidden className="hidden text-2xl lg:block" style={{ color: C.faint }}>→</div>
          <div style={{ background: C.ink, borderRadius: 12, color: C.card, padding: 16, textAlign: "center" }}><p style={{ color: "#B8B2A9", fontSize: 10.5 }}>Modeled avg / year</p><p className="mt-1" style={{ color: "#8FD0A5", fontFamily: MONO, fontSize: 27, fontWeight: 800 }}>{slp ? formatReturn(slp.apy) : "—"}</p></div>
        </div>
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[.65fr_1.35fr] lg:items-center" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "20px clamp(16px,2.5vw,28px)" }}>
        <div>
          ...
          <h2 className="mt-2" style={{ color: C.text, fontSize: 22, fontWeight: 550, letterSpacing: "-0.03em" }}>JT absorbs source losses before ST.</h2>
          <p className="mt-3" style={{ color: C.muted, fontSize: 11.5, lineHeight: 1.5 }}>This is ST/JT context. SLP separately faces pool-price and trading risk.</p>
        </div>
        <div>
          <div className="flex items-end justify-between gap-4"><div><p style={{ color: C.green, fontFamily: MONO, fontSize: 24, fontWeight: 800 }}>{protectionPct.toFixed(1)}%</p><p style={{ color: C.muted, fontSize: 11.5 }}>source loss before 100 ST starts falling</p></div><Badge tone="context">Market context</Badge></div>
          <div aria-label={`JT absorbs source losses through ${protectionPct.toFixed(1)} percent before ST starts falling`} className="mt-4 overflow-hidden rounded-full" role="img" style={{ background: C.dangerSoft, height: 18 }}><div style={{ background: C.green, height: "100%", width: `${Math.min(100, (coverage.coverageLossLimit / coverage.displayMaxLoss) * 100)}%` }} /></div>
          <div className="mt-2 flex justify-between gap-3" style={{ color: C.faint, fontSize: 10 }}><span>JT absorbs loss</span><span>ST absorbs loss after the buffer</span></div>
        </div>
      </section>

      <details style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "15px clamp(16px,2.5vw,24px)" }}>
        <summary style={{ color: C.text, cursor: "pointer", fontSize: 13.5, fontWeight: 650 }}>See source, ST, and JT context</summary>
        <div className="mt-4 rounded-xl" style={{ background: C.page, border: `1px solid ${C.border}`, padding: 14 }}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><Eyebrow>Source baseline · {assetName}</Eyebrow><p className="mt-1" style={{ color: C.muted, fontSize: 12 }}>Untranched source—not an SLP result.</p></div><div className="flex flex-wrap gap-x-5 gap-y-1" style={{ fontFamily: MONO, fontSize: 12 }}><span>{sourceEndValue}</span><span>{formatReturn(sourceApy)} / year</span><span>{formatDrawdown(sourceMaxDrawdown)} worst drop</span></div></div>
          <p className="mt-2" style={{ color: C.muted, fontSize: 11 }}>{dataSummary}</p>
        </div>
        <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
          {marketPositions.map((position) => <article key={position.symbol} style={{ border: `1px solid ${C.border}`, borderRadius: 12, padding: 14 }}><div className="flex items-center justify-between"><span style={{ color: position.color, fontFamily: MONO, fontWeight: 850 }}>{position.symbol}</span><span style={{ color: C.muted, fontSize: 10.5 }}>{position.name}</span></div><p className="mt-2" style={{ color: C.text, fontSize: 13.5, fontWeight: 650 }}>{position.job}</p><p className="mt-1" style={{ color: C.muted, fontSize: 11.5 }}>{position.paidFor}</p><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1" style={{ color: C.muted, fontSize: 10.5 }}><span>End <strong style={{ color: C.text, fontFamily: MONO }}>{position.endValue}</strong></span><span>Avg/year <strong style={{ color: C.text, fontFamily: MONO }}>{formatReturn(position.apy)}</strong></span><span>Worst drop <strong style={{ color: position.maxDrawdown > 0.0005 ? C.danger : C.text, fontFamily: MONO }}>{formatDrawdown(position.maxDrawdown)}</strong></span></div></article>)}
        </div>
      </details>
      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: "17px clamp(16px,2.5vw,24px)" }}>
        <div><Eyebrow>Need the full model?</Eyebrow><p className="mt-1.5" style={{ color: C.text, fontSize: 14, fontWeight: 650 }}>Open the detailed simulator for hoverable curves and history.</p></div>
        <a href={`/day-sim?market=${encodeURIComponent(marketId)}`} style={{ background: C.rust, borderRadius: 9, color: C.card, fontSize: 12.5, fontWeight: 700, padding: "10px 13px", textDecoration: "none", whiteSpace: "nowrap" }}>Open detailed simulator →</a>
      </section>
    </div>
  );
}
